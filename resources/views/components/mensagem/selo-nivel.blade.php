...
{{-- Badge de NÍVEL de visibilidade: pílula AA (fundo translúcido + texto escurecido) + cadeado se restrito.
     ...
     Prop `escuro`: variante para superfícies ESCURAS (hero navy do single) — pílula translúcida
     branca com texto branco (AA garantido), hue só no ponto/cadeado. Caminho claro (cards/lista/
     legenda) permanece INALTERADO. --}}
@props(['visibilidade', 'escuro' => false])
@if ($visibilidade)
    @if ($escuro)
        {{-- Superfície ESCURA (hero navy): pílula translúcida branca; hue só no ponto/cadeado. AA garantido (white sobre navy). --}}
        <span {{ $attributes->merge(['class' => 'inline-flex items-center gap-1.5 rounded-pill bg-white/12 px-2.5 py-1 text-[11px] font-semibold text-white/90']) }}>
    ...
    @else
        {{-- Superfície CLARA (cards/lista): pílula AA fundo translúcido + texto escurecido (INALTERADO). --}}
